...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const nodosPlanta0=[
        ...
        { id: 'nodo_escalera_bloque_B_segundaria', latlng: [626.528845, 488.818359],  name: 'nodo escalera bloque B segundaria' },
        { id: 'nodo_escalera_bloque_C', latlng: [421.415429, 408.658203],  name: 'nodo escalera bloque C' },
        { id: 'nodo_escalera_bloque_C_norte', latlng: [461.198437, 414.850586],  name: 'nodo escalera bloque C norte' },
        { id: 'nodo_escalera_bloque_C_sur', latlng: [376.716109, 405.686035],  name: 'nodo escalera bloque C sur' },
    ...
    const edgesPlanta0 = [
        ...
        { from: 'nodo_final_pasillo_asociaciones', to: 'nodo_escalera_bloque_B_segundaria', weight:  getDistanceBetweenPoints('nodo_final_pasillo_asociaciones','nodo_escalera_bloque_B_segundaria', nodosPlanta0)},

        { from: 'nodo_escalera_bloque_C', to: 'nodo_escalera_bloque_C_norte', weight:  getDistanceBetweenPoints('nodo_escalera_bloque_C','nodo_escalera_bloque_C_norte', nodosPlanta0)},
        ...
        { from: 'nodo_escalera_bloque_C', to: 'nodo_escalera_bloque_C_sur', weight:  getDistanceBetweenPoints('nodo_escalera_bloque_C','nodo_escalera_bloque_C_sur', nodosPlanta0)},
    
        { from: 'nodo_escalera_bloque_C_sur', to: 'C006', weight: getDistanceBetweenPoints('nodo_escalera_bloque_C_sur','C006', nodosPlanta0) },
        { from: 'nodo_escalera_bloque_C_sur', to: 'C007', weight:  getDistanceBetweenPoints('nodo_escalera_bloque_C_sur','C007', nodosPlanta0)},
        ...
        { from: 'nodo_escalera_bloque_C_norte', to: 'C001', weight:  getDistanceBetweenPoints('nodo_escalera_bloque_C_norte','C001', nodosPlanta0)},
        { from: 'nodo_escalera_bloque_C_norte', to: 'C002', weight:  getDistanceBetweenPoints('nodo_escalera_bloque_C_norte','C002', nodosPlanta0)},   
    ]
...
//GET DISTANCE BETWEEN NODES
    function getDistanceBetweenPoints(id1, id2, nodosPlanta) {
        ...
        if (!point1 || !point2) {
            console.error('One or both IDs are not found in the array');
            return null;
        }
...
//--------------------------------------------------------------------------------------------------------
    ...
    //     planta0.forEach(point => {
    //         distances[point.id] = Infinity;
    //         previous[point.id] = null;
    //         queue.enqueue(point.id, Infinity);
    //     });
    //     distances[startId] = 0;
    ...
    //         const neighbors = edgesPlanta0.filter(edge => edge.from === currentId);
    //         neighbors.forEach(neighbor => {
    //             const alt = distances[currentId] + neighbor.weight;
    //             if (alt < distances[neighbor.to]) {
    //                 distances[neighbor.to] = alt;
    //                 previous[neighbor.to] = currentId;
    //                 queue.enqueue(neighbor.to, alt);
    //             }
    //         });
    //     }
    ...
    //         u = previous[u];
    //     }
    //     if (path.length) path.unshift(startId);
    //     return path;
    // }

    // Priority Queue class (simple implementation)
    // class PriorityQueue {
    //     constructor() {
    //         this.items = [];
    //     }
    ...
    findRouteButton.addEventListener('click', () => {
        ...
        const latlngs = path.map(id => {
            const point = nodosPlanta0.find(p => p.id === id);
            return point.latlng;
        });
        ...
        map.fitBounds(window.currentRoute.getBounds());
         ...
         // Remove existing start and end markers if any
        ...
        const startLatlng = nodosPlanta0.find(point => point.id === startPoint).latlng;
        const endLatlng = nodosPlanta0.find(point => point.id === endPoint).latlng;
